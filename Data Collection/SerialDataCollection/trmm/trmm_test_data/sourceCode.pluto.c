...
/* Start of CLooG code */
if ((PS1 >= 1) && (PS2 >= 2) && (PS3 >= 1)) {
  for (t1=0;t1<=floord(PS3-1,512);t1++) {
    for (t2=0;t2<=min(floord(PS1-1,512),floord(PS2-2,512));t2++) {
      for (t3=t2;t3<=floord(PS2-1,512);t3++) {
        for (t4=512*t2;t4<=min(min(min(PS1-1,PS2-2),512*t2+511),512*t3+510);t4++) {
          for (t5=max(512*t3,t4+1);t5<=min(PS2-1,512*t3+511);t5++) {
            lbv=512*t1;
            ubv=min(PS3-1,512*t1+511);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              B[t4][t6] += A[t5][t4] * B[t5][t6];;
              B[t4][t6]=alpha*B[t4][t6];;
            }
          }
        }
      }
