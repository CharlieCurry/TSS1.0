...
big_dataframe=pd.concat([predict_best[['real_time','I','predict_time','size1','size2','size3']],real_best[['real_time','I']]],axis=1)
...
#P['T_p32'] = P['T_p32']*(labelmax-labelmin)+labelmin
P['To'] = P['To']*(labelmax-labelmin)+labelmin
print(P.head())
P = P.loc[(P['T']>P['To'])]

P['p_32']=P.apply(lambda x:(x['T']-x['T_32'])/(x['T']-x['To']),axis=1)
#P['p_p32']=P.apply(lambda x:(x['T']-x['T_p32'])/(x['T']-x['To']),axis=1)
P['speedup=T/T_32'] = P.apply(lambda x:x['T']/x['T_32'],axis=1)
#P['speedup=T/T_p32'] = P.apply(lambda x:x['T']/x['T_p32'],axis=1)
P['T_32/T_O'] = P.apply(lambda x:x['T_32']/x['To'],axis=1)
#P['T_p32/T_O'] = P.apply(lambda x:x['T_p32']/x['To'],axis=1)
#P = P.loc[(P['p_32']>=0)&(P['p_32']<=10)]
print(P.describe())

print("--------------------------------")
print("p_32:",P['p_32'].mean())
#print("p_p32:",P['p_p32'].mean())
print("speedup=T/T_32:",P['speedup=T/T_32'].mean())
#print("speedup=T/T_p32:",P['speedup=T/T_p32'].mean())
print("T_32/T_O:",P['T_32/T_O'].mean())
#print("T_p32/T_O:",P['T_p32/T_O'].mean())
print("--------------------------------")