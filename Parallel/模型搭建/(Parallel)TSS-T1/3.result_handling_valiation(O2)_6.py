...
for i in range(int(kinds)):
    ...
    # base = base.append(kind1_dataframe[kind1_dataframe['I']==32],ignore_index=True)
    # base = base.append(kind1_dataframe[kind1_dataframe['I'] == 32], ignore_index=True)
    predict_best = predict_best.append(df_group[0:1],ignore_index=True)
...
predict = pd.DataFrame(columns=['predict_time','size1','size2','size3','I','j-0','j-i','k-0','k-i','k-j','L3_RP','L3_RNP','L3_RI','L3_WP','L3_WNP','L3WI','L2_RP','L2_RNP','L2_RI','L2_WP','L2_WNP','L2WI','L1_RP','L1_RNP','L1_RI','L1_WP','L1_WNP','L1WI','real_time'],dtype='float')
...
print("oracle columns",oracle.columns)
for i in range(len(predict_best)):
    a = predict_best.loc[i]
    d = pd.DataFrame(a).T
    predict = predict.append([d]*6)
# print(oracle)
#print("oralce shape",predict.shape)
predict = predict.reset_index(drop=True)
print("predict columns",predict.columns)
print(predict['real_time'].head())
print(oracle['real_time'].head())
...
big_dataframe=pd.concat([predict[['real_time','I','predict_time','size1','size2','size3']],oracle[['real_time','I']]],axis=1).astype('float')
